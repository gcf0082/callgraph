...
import com.gcf.callgraph.jacg.dto.annotation.AnnotationInfo4Write;
import com.gcf.callgraph.jacg.dto.entity.JarInfoEntity;
import com.gcf.callgraph.jacg.dto.entity.MethodCallEntity;
...
import com.gcf.callgraph.jacg.util.SqlUtil;
...
import com.gcf.callgraph.javacg.extensions.dto.ExtendedData;
...
public class RunnerWriteDb extends AbstractRunner {
    ...
    private boolean createTables() {
        ...
        String handled_classNameSql = readCreateTableSql(JACGConstants.DIR_SQL + File.separator + JACGConstants.FILE_SQL_HANDLED_CLASS_NAME);
        ...
        if (!dbOperator.createTable(classNameSql) ||
                ...
                !dbOperator.createTable(handled_classNameSql) ||
    ...
    private boolean writeHandledClassName2Db() {

        Set<String> handledClassNameSet = jCallGraph.getHandledClassNameSet();
        if (handledClassNameSet.isEmpty()) {
            return true;
        }

        logger.info("写入数据库，保存处理过的类名信息表 {}", handledClassNameSet.size());

        String sqlKey = JACGConstants.SQL_KEY_INSERT_HANDLED_CLASS_NAME;
        String sql = sqlCacheMap.get(sqlKey);
        if (sql == null) {
            sql = genAndCacheInsertSql(sqlKey,
                    false,
                    JACGConstants.TABLE_PREFIX_HANDLED_CLASS_NAME,
                    JACGConstants.TABLE_COLUMNS_HANDLED_CLASS_NAME);
        }

        List<Object[]> objectList = new ArrayList<>(handledClassNameSet.size());
        for (String fullClassName : handledClassNameSet) {
            String simpleClassName = JACGUtil.getSimpleClassNameFromFull(fullClassName);
