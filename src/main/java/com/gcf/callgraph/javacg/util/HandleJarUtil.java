package com.gcf.callgraph.javacg.util;

import com.gcf.callgraph.javacg.common.JavaCGConstants;
import com.gcf.callgraph.javacg.dto.JarInfo;
...
public class HandleJarUtil {
    ...
    public static String handleJar(String[] jarOrDirPathArray, Map<String, JarInfo> jarInfoMap) {
        if (jarOrDirPathArray.length == 1) {
            ...
            String oneFilePath = com.gcf.callgraph.javacg.util.FileUtil.getCanonicalPath(oneFile);
    ...
    private static String mergeJar(String[] jarOrDirPathArray, Map<String, JarInfo> jarInfoMap) {
        ...
        if (newJarFile.exists()) {
            ...
            if (newJarFile.isDirectory()) {
                System.err.println("新的jar包文件已存在，但是是目录: " + com.gcf.callgraph.javacg.util.FileUtil.getCanonicalPath(newJarFile));
                return null;
            } else if (!com.gcf.callgraph.javacg.util.FileUtil.deleteFile(newJarFile)) {
                System.err.println("新的jar包文件已存在，删除失败: " + com.gcf.callgraph.javacg.util.FileUtil.getCanonicalPath(newJarFile));
                return null;
            }
        }
        ...
        try (ZipOutputStream zos = new ZipOutputStream(new FileOutputStream(newJarFile))) {
            ...
            for (File jarFileOrDir : jarFileOrDirList) {
                ...
                if (destJarDirNameSet.contains(jarFileOrDirName)) {
                    System.err.println("指定的jar/war包或目录存在同名，不处理: " + jarFileOrDirName + " " + com.gcf.callgraph.javacg.util.FileUtil.getCanonicalPath(jarFileOrDir));
                    continue;
                }
            ...
            for (File jarFileInDir : jarFileInDirList) {
                ...
                String jarCanonicalPath = com.gcf.callgraph.javacg.util.FileUtil.getCanonicalPath(jarFileInDir);
                // 记录jar包信息，不覆盖现有值
            ...
            return com.gcf.callgraph.javacg.util.FileUtil.getCanonicalPath(newJarFile);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
                ...
                if (!mergeClassInJarPackageList.isEmpty()) {
                    ...
                    for (String mergeClassInJarPackage : mergeClassInJarPackageList) {
                        if (jarEntryName.startsWith(mergeClassInJarPackage) || jarEntryName.startsWith("WEB-INF/classes/" + mergeClassInJarPackage)) {
                            ...
                            com.gcf.callgraph.javacg.util.JavaCGUtil.debugPrint("当前class文件包名匹配，需要合并 " + jarEntryName + " " + mergeClassInJarPackage);
                            break;
                        }
                    }
